...
class Main:
    ...
    def grow(self,d0):
       ...
       from entropydown import entropy
       
       Body = DoublyLinkedList()
       Body.insert_at_beg(Node([30,5]),0)
...
#       x1 = np.arange(0,0.5,d+1)
       for d in range(1,d0):  #range(1,d0):
           ...
           for i in range(0,d+1):# range(0,d+1): #np.arange(0,0.5,d+1)
               ...
               if i==d:
                   Body.insert_at_end(Node([30+i,5]),i)    
                       
               else:
                   for j in [-k,k]:
                       if j<0:
                           Body.insert_at_beg(Node([30+i,j+5]),i)

                       else:
                           Body.insert_at_end(Node([30+i,j+5]),i)
